...
- `index` — индексация директории, поддерживает `--watch`, `--deep-deps`, `--durability fast|safe`, `--profile eco|balanced|max`, `--threads N`, `--throttle-ms`; пайплайн разделён на `cold-run` и `incremental`:
  ...
  - incremental: параллельный single-read precheck, parse через `parse_source_cached(&content, ...)` без повторного disk-read.
...
- `index_workspace` — индексация проекта. Params: `path`, `watch`, `include_deps`.
  ...
  - как и CLI, использует split `cold-run/incremental`: cold-run без precheck и без changed cleanup, incremental с parallel hash precheck и parse из in-memory content для changed-файлов.
